...
func (bt BitcoinTransaction) Release(t Transaction, comment, userUuid string) error {
	...
	vendor, err := FindUserByUuid(t.SellerUuid)
	if err != nil {
		return err
	}
	...
	// Vendor address
	addressTo := vendor.Bitcoin
	if vendor.Bitcoin == "" { // if seller doesn't have auto-withrawal wallet set it up
		addressTo = vendor.FindRecentBitcoinWallet().PublicKey
	}
