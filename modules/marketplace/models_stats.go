...
func CountNumberOfReleasedTransactionsForVendor(userUuid string) int {
	var (
		btcQuery = `select tx_number as count from v_vendor_bitcoin_tx_stats where seller_uuid=?`
		bchQuery = `select tx_number as count from v_vendor_bitcoin_cash_tx_stats where seller_uuid=?`
		ethQuery = `select tx_number as count from v_vendor_ethereum_tx_stats where seller_uuid=?`
	)
