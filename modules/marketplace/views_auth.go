...
func (c *Context) ViewVerificationPlanGET(w web.ResponseWriter, r *web.Request) {
	...
	util.RenderTemplate(w, "auth/verification_account", c)
}
...
func (c *Context) ViewVerificationPlanPurchaseGET(w web.ResponseWriter, r *web.Request) {
	...
	if c.ViewUser.IsBronzeAccount && (c.Account == "silver" || c.Account == "gold") {
		c.PriceUSD -= 100
	}
	...
	util.RenderTemplate(w, "auth/verification_account_purchase", c)
}
...
func (c *Context) ViewVerificationPlanPurchasePOST(w web.ResponseWriter, r *web.Request) {
	...
	switch c.Account {
	case "gold":
		c.ViewUser.User.IsGoldAccount = true
		c.ViewUser.User.IsSilverAccount = false
		c.ViewUser.User.IsBronzeAccount = false
		c.ViewUser.User.IsFreeAccount = false
	case "silver":
		c.ViewUser.User.IsGoldAccount = false
		c.ViewUser.User.IsSilverAccount = true
		c.ViewUser.User.IsBronzeAccount = false
		c.ViewUser.User.IsFreeAccount = false
	case "bronze":
		c.ViewUser.User.IsGoldAccount = false
		c.ViewUser.User.IsSilverAccount = false
		c.ViewUser.User.IsBronzeAccount = true
		c.ViewUser.User.IsFreeAccount = false
	}
...
func (c *Context) ViewVerificationEncryptionGET(w web.ResponseWriter, r *web.Request) {
	util.RenderTemplate(w, "auth/verification_encryption", c)
}
...
func (c *Context) ViewVerificationAgreementGET(w web.ResponseWriter, r *web.Request) {
	...
	util.RenderTemplate(w, "auth/verification_agreement", c)
}
...
func (c *Context) ViewVerificationAgreementPOST(w web.ResponseWriter, r *web.Request) {
	...
	if !isSignatureCorrect || err != nil {
		...
		c.ViewUser.User.SignedAgreementText = signedAgreement
		c.ViewUser.User.IsSigned = true
		c.ViewUser.User.AgreementDate = time.Now()
		c.ViewUser.User.Save()
	}
...
func (c *Context) ViewVerificationThreadGET(w web.ResponseWriter, r *web.Request) {
	...
	util.RenderTemplate(w, "auth/verification_thread", c)
}
...
func (c *Context) ProfileGET(w web.ResponseWriter, r *web.Request) {
	...
	if c.ViewUser.IsSeller {
		c.ViewUser.User, _ = FindUserByUuid(c.ViewUser.Uuid)
	}
