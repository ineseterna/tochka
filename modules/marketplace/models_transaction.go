...
type Transaction struct {
	...
	BitcoinTransactionUuid     string `json:"bitcoin_transcation_uuid,omitempty" sql:"index"`
	BitcoinCashTransactionUuid string `json:"bitcoin_cash_transcation_uuid,omitempty" sql:"index"`
	EthereumTransactionUuid    string `json:"ethereum_transcation_uuid,omitempty" sql:"index"`
	...
	BitcoinTransaction     *BitcoinTransaction     `json:"bitcoin_transcation,omitempty"`
	EthereumTransaction    *EthereumTransaction    `json:"ethereum_transcation,omitempty"`
	BitcoinCashTransaction *BitcoinCashTransaction `json:"bitcoin_cash_transcation,omitempty"`
}
...
func CreateTransactionForPackage(
	...
	var (
		status        = []TransactionStatus{TransactionStatus{Time: time.Now(), Amount: 0, Status: "PENDING"}}
...
func FindDisputedTransactionsForUser(userUuid string) []Transaction {
	var transactions []Transaction
	database.
		Table("v_current_cummulative_transaction_statuses").
		Joins("JOIN disputes on disputes.uuid=v_current_cummulative_transaction_statuses.dispute_uuid").
		Where("(seller_uuid=? OR buyer_uuid=?)", userUuid, userUuid).
		Order("disputes.created_at DESC").
		Preload("Dispute").
		Preload("Dispute.ResolverUser").
		Preload("ShippingStatus").
...
func CountPayedTransactionsForBuyerAndSeller(buyerUuid, sellerUuid string) int {
	...
	database.
		Table("v_current_cummulative_transaction_statuses").
		Where("(buyer_uuid=? and seller_uuid=?)", buyerUuid, sellerUuid).
